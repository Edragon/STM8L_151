 ...
 185                     ; 58   FLASH_Unlock(FLASH_MemType_Data);//先解锁，注意参数
 187  0063 a6f7          	ld	a,#247
 188  0065 cd0000        	call	_FLASH_Unlock
 190                     ; 59   FLASH_EraseByte(0x1000);         //再擦除
 192  0068 ae1000        	ldw	x,#4096
 193  006b 89            	pushw	x
 194  006c ae0000        	ldw	x,#0
 195  006f 89            	pushw	x
 196  0070 cd0000        	call	_FLASH_EraseByte
 198  0073 5b04          	addw	sp,#4
 199                     ; 60   FLASH_ProgramByte(0x1000,0x0f);  //写数据，第一个参数为地址，第二个参数为写入的数据，1000为片内EEPROM区域的起始位置
 201  0075 4b0f          	push	#15
 202  0077 ae1000        	ldw	x,#4096
 203  007a 89            	pushw	x
 204  007b ae0000        	ldw	x,#0
 205  007e 89            	pushw	x
 206  007f cd0000        	call	_FLASH_ProgramByte
 208  0082 5b05          	addw	sp,#5
 209                     ; 62   r_byte = FLASH_ReadByte(0x1000);//读指定地址的数据
 211  0084 ae1000        	ldw	x,#4096
 212  0087 89            	pushw	x
 ...
 219                     ; 63   USART_SendData8 (USART1,r_byte);//通过UART发送，16进制
