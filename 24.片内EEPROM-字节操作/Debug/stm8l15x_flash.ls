 ...
 693  008b cd01e6        	call	_FLASH_WaitForLastOperation
 ...
 755  009b cd01e6        	call	_FLASH_WaitForLastOperation
...
1325                     ; 730 {
1326                     	switch	.text
1327  012c               _FLASH_GetPowerStatus:
1331                     ; 731   return((FLASH_PowerStatus_TypeDef)(FLASH->CR1 & (uint8_t)0x0C));
1333  012c c65050        	ld	a,20560
1334  012f a40c          	and	a,#12
1337  0131 81            	ret
...
1441                     ; 751 {
1442                     	switch	.text
1443  0132               _FLASH_ProgramBlock:
1445  0132 89            	pushw	x
1446  0133 5206          	subw	sp,#6
1447       00000006      OFST:	set	6
1450                     ; 752   uint16_t Count = 0;
1452  0135 1e05          	ldw	x,(OFST-1,sp)
1453                     ; 753   uint32_t startaddress = 0;
1455  0137 96            	ldw	x,sp
1456  0138 1c0001        	addw	x,#OFST-5
1457  013b cd0000        	call	c_ltor
...
1465  013e 7b0b          	ld	a,(OFST+5,sp)
1466  0140 a1fd          	cp	a,#253
1467  0142 260c          	jrne	L136
1468                     ; 760   assert_param(IS_FLASH_PROGRAM_BLOCK_NUMBER(BlockNum));
1470                     ; 761     startaddress = FLASH_PROGRAM_START_PHYSICAL_ADDRESS;
1472  0144 ae8000        	ldw	x,#32768
1473  0147 1f03          	ldw	(OFST-3,sp),x
1474  0149 ae0000        	ldw	x,#0
1475  014c 1f01          	ldw	(OFST-5,sp),x
1477  014e 200a          	jra	L336
1478  0150               L136:
...
1483  0150 ae1000        	ldw	x,#4096
1484  0153 1f03          	ldw	(OFST-3,sp),x
1485  0155 ae0000        	ldw	x,#0
1486  0158 1f01          	ldw	(OFST-5,sp),x
1487  015a               L336:
1488                     ; 770   startaddress = startaddress + ((uint32_t)BlockNum * FLASH_BLOCK_SIZE);
1490  015a 1e07          	ldw	x,(OFST+1,sp)
1491  015c a680          	ld	a,#128
1492  015e cd0000        	call	c_cmulx
1494  0161 96            	ldw	x,sp
1495  0162 1c0001        	addw	x,#OFST-5
1496  0165 cd0000        	call	c_lgadd
1498                     ; 773   if (FLASH_ProgMode == FLASH_ProgramMode_Standard)
1500  0168 0d0c          	tnz	(OFST+6,sp)
1501  016a 2606          	jrne	L536
1502                     ; 776   FLASH->CR2 |= FLASH_CR2_PRG;
1504  016c 72105051      	bset	20561,#0
1506  0170 2004          	jra	L736
1507  0172               L536:
1508                     ; 781   FLASH->CR2 |= FLASH_CR2_FPRG;
1510  0172 72185051      	bset	20561,#4
1511  0176               L736:
1512                     ; 785   for (Count = 0; Count < FLASH_BLOCK_SIZE; Count++)
1514  0176 5f            	clrw	x
1515  0177 1f05          	ldw	(OFST-1,sp),x
1516  0179               L146:
1517                     ; 788   *((PointerAttr uint8_t*) (uint16_t)startaddress + Count) = ((uint8_t)(Buffer[Count]));
1519  0179 1e0d          	ldw	x,(OFST+7,sp)
1520  017b 72fb05        	addw	x,(OFST-1,sp)
1521  017e f6            	ld	a,(x)
1522  017f 1e03          	ldw	x,(OFST-3,sp)
1523  0181 72fb05        	addw	x,(OFST-1,sp)
1524  0184 f7            	ld	(x),a
1525                     ; 785   for (Count = 0; Count < FLASH_BLOCK_SIZE; Count++)
1527  0185 1e05          	ldw	x,(OFST-1,sp)
1528  0187 1c0001        	addw	x,#1
1529  018a 1f05          	ldw	(OFST-1,sp),x
1532  018c 1e05          	ldw	x,(OFST-1,sp)
1533  018e a30080        	cpw	x,#128
1534  0191 25e6          	jrult	L146
1535                     ; 793 }
1538  0193 5b08          	addw	sp,#8
1539  0195 81            	ret
1602                     ; 805 IN_RAM(void FLASH_EraseBlock(uint16_t BlockNum, FLASH_MemType_TypeDef FLASH_MemType))
1602                     ; 806 {
1603                     	switch	.text
1604  0196               _FLASH_EraseBlock:
1606  0196 89            	pushw	x
1607  0197 5206          	subw	sp,#6
1608       00000006      OFST:	set	6
1611                     ; 807   uint32_t startaddress = 0;
1613  0199 96            	ldw	x,sp
1614  019a 1c0003        	addw	x,#OFST-3
1615  019d cd0000        	call	c_ltor
1617                     ; 815   assert_param(IS_FLASH_MEMORY_TYPE(FLASH_MemType));
1619                     ; 816   if (FLASH_MemType == FLASH_MemType_Program)
1621  01a0 7b0b          	ld	a,(OFST+5,sp)
1622  01a2 a1fd          	cp	a,#253
1623  01a4 260c          	jrne	L107
1624                     ; 818   assert_param(IS_FLASH_PROGRAM_BLOCK_NUMBER(BlockNum));
1626                     ; 819     startaddress = FLASH_PROGRAM_START_PHYSICAL_ADDRESS;
1628  01a6 ae8000        	ldw	x,#32768
1629  01a9 1f05          	ldw	(OFST-1,sp),x
1630  01ab ae0000        	ldw	x,#0
1631  01ae 1f03          	ldw	(OFST-3,sp),x
1633  01b0 200a          	jra	L307
1634  01b2               L107:
...
1639  01b2 ae1000        	ldw	x,#4096
1640  01b5 1f05          	ldw	(OFST-1,sp),x
1641  01b7 ae0000        	ldw	x,#0
1642  01ba 1f03          	ldw	(OFST-3,sp),x
1643  01bc               L307:
1644                     ; 829   pwFlash = (PointerAttr uint32_t *)(uint16_t)(startaddress + ((uint32_t)BlockNum * FLASH_BLOCK_SIZE));
1646  01bc 1e07          	ldw	x,(OFST+1,sp)
1647  01be a680          	ld	a,#128
1648  01c0 cd0000        	call	c_cmulx
1650  01c3 96            	ldw	x,sp
1651  01c4 1c0003        	addw	x,#OFST-3
1652  01c7 cd0000        	call	c_ladd
1654  01ca be02          	ldw	x,c_lreg+2
1655  01cc 1f01          	ldw	(OFST-5,sp),x
1656                     ; 835   FLASH->CR2 |= FLASH_CR2_ERASE;
1658  01ce 721a5051      	bset	20561,#5
1659                     ; 838   *pwFlash = (uint32_t)0;
1661  01d2 1e01          	ldw	x,(OFST-5,sp)
1662  01d4 a600          	ld	a,#0
1663  01d6 e703          	ld	(3,x),a
1664  01d8 a600          	ld	a,#0
1665  01da e702          	ld	(2,x),a
1666  01dc a600          	ld	a,#0
1667  01de e701          	ld	(1,x),a
1668  01e0 a600          	ld	a,#0
1669  01e2 f7            	ld	(x),a
1670                     ; 845 }
1673  01e3 5b08          	addw	sp,#8
1674  01e5 81            	ret
1759                     ; 857 IN_RAM(FLASH_Status_TypeDef FLASH_WaitForLastOperation(FLASH_MemType_TypeDef FLASH_MemType))
1759                     ; 858 {
1760                     	switch	.text
1761  01e6               _FLASH_WaitForLastOperation:
1763  01e6 5205          	subw	sp,#5
1764       00000005      OFST:	set	5
1767                     ; 859   uint32_t timeout = OPERATION_TIMEOUT;
1769  01e8 aeffff        	ldw	x,#65535
1770  01eb 1f03          	ldw	(OFST-2,sp),x
1771  01ed ae0000        	ldw	x,#0
1772  01f0 1f01          	ldw	(OFST-4,sp),x
1773                     ; 860   uint8_t flagstatus = 0x00;
1775  01f2 0f05          	clr	(OFST+0,sp)
1776                     ; 862   if (FLASH_MemType == FLASH_MemType_Program)
1778  01f4 a1fd          	cp	a,#253
1779  01f6 2631          	jrne	L367
1781  01f8 2010          	jra	L157
1782  01fa               L747:
1783                     ; 866       flagstatus = (uint8_t)(FLASH->IAPSR & (uint8_t)(FLASH_IAPSR_EOP |
1783                     ; 867                              FLASH_IAPSR_WR_PG_DIS));
1785  01fa c65054        	ld	a,20564
1786  01fd a405          	and	a,#5
1787  01ff 6b05          	ld	(OFST+0,sp),a
1788                     ; 868       timeout--;
1790  0201 96            	ldw	x,sp
1791  0202 1c0001        	addw	x,#OFST-4
1792  0205 a601          	ld	a,#1
1793  0207 cd0000        	call	c_lgsbc
1795  020a               L157:
1796                     ; 864   while ((flagstatus == 0x00) && (timeout != 0x00))
1798  020a 0d05          	tnz	(OFST+0,sp)
1799  020c 2628          	jrne	L757
1801  020e 96            	ldw	x,sp
1802  020f 1c0001        	addw	x,#OFST-4
1803  0212 cd0000        	call	c_lzmp
1805  0215 26e3          	jrne	L747
1806  0217 201d          	jra	L757
1807  0219               L167:
1808                     ; 875       flagstatus = (uint8_t)(FLASH->IAPSR & (uint8_t)(FLASH_IAPSR_HVOFF |
1808                     ; 876                              FLASH_IAPSR_WR_PG_DIS));
1810  0219 c65054        	ld	a,20564
1811  021c a441          	and	a,#65
1812  021e 6b05          	ld	(OFST+0,sp),a
1813                     ; 877       timeout--;
1815  0220 96            	ldw	x,sp
1816  0221 1c0001        	addw	x,#OFST-4
1817  0224 a601          	ld	a,#1
1818  0226 cd0000        	call	c_lgsbc
1820  0229               L367:
1821                     ; 873     while ((flagstatus == 0x00) && (timeout != 0x00))
1823  0229 0d05          	tnz	(OFST+0,sp)
1824  022b 2609          	jrne	L757
1826  022d 96            	ldw	x,sp
1827  022e 1c0001        	addw	x,#OFST-4
1828  0231 cd0000        	call	c_lzmp
1830  0234 26e3          	jrne	L167
1831  0236               L757:
1832                     ; 880   if (timeout == 0x00 )
1834  0236 96            	ldw	x,sp
1835  0237 1c0001        	addw	x,#OFST-4
1836  023a cd0000        	call	c_lzmp
1838  023d 2604          	jrne	L177
1839                     ; 882   flagstatus = FLASH_Status_TimeOut;
1841  023f a602          	ld	a,#2
1842  0241 6b05          	ld	(OFST+0,sp),a
1843  0243               L177:
1844                     ; 885   return((FLASH_Status_TypeDef)flagstatus);
1846  0243 7b05          	ld	a,(OFST+0,sp)
1849  0245 5b05          	addw	sp,#5
1850  0247 81            	ret
