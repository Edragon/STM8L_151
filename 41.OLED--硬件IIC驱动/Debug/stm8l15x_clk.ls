  ...
  19  0003 08            	dc.b	8
  20  0004 10            	dc.b	16
  49                     ; 114 void CLK_DeInit(void)
  49                     ; 115 {
  51                     	switch	.text
  52  0000               _CLK_DeInit:
  56                     ; 116   CLK->ICKCR = CLK_ICKCR_RESET_VALUE;
  58  0000 351150c2      	mov	20674,#17
  59                     ; 117   CLK->ECKCR = CLK_ECKCR_RESET_VALUE;
  61  0004 725f50c6      	clr	20678
  62                     ; 118   CLK->CRTCR = CLK_CRTCR_RESET_VALUE;
  64  0008 725f50c1      	clr	20673
  65                     ; 119   CLK->CBEEPR = CLK_CBEEPR_RESET_VALUE;
  67  000c 725f50cb      	clr	20683
  68                     ; 120   CLK->SWR  = CLK_SWR_RESET_VALUE;
  70  0010 350150c8      	mov	20680,#1
  71                     ; 121   CLK->SWCR = CLK_SWCR_RESET_VALUE;
  73  0014 725f50c9      	clr	20681
  74                     ; 122   CLK->CKDIVR = CLK_CKDIVR_RESET_VALUE;
  76  0018 350350c0      	mov	20672,#3
  77                     ; 123   CLK->PCKENR1 = CLK_PCKENR1_RESET_VALUE;
  79  001c 725f50c3      	clr	20675
  80                     ; 124   CLK->PCKENR2 = CLK_PCKENR2_RESET_VALUE;
  82  0020 358050c4      	mov	20676,#128
  83                     ; 125   CLK->PCKENR3 = CLK_PCKENR3_RESET_VALUE;
  85  0024 725f50d0      	clr	20688
  86                     ; 126   CLK->CSSR  = CLK_CSSR_RESET_VALUE;
  88  0028 725f50ca      	clr	20682
  89                     ; 127   CLK->CCOR = CLK_CCOR_RESET_VALUE;
  91  002c 725f50c5      	clr	20677
  92                     ; 128   CLK->HSITRIMR = CLK_HSITRIMR_RESET_VALUE;
  94  0030 725f50cd      	clr	20685
  95                     ; 129   CLK->HSICALR = CLK_HSICALR_RESET_VALUE;
  97  0034 725f50cc      	clr	20684
  98                     ; 130   CLK->HSIUNLCKR = CLK_HSIUNLCKR_RESET_VALUE;
 100  0038 725f50ce      	clr	20686
 101                     ; 131   CLK->REGCSR = CLK_REGCSR_RESET_VALUE;
 103  003c 35b950cf      	mov	20687,#185
 104                     ; 132 }
 107  0040 81            	ret
 162                     ; 154 void CLK_HSICmd(FunctionalState NewState)
 162                     ; 155 {
 163                     	switch	.text
 164  0041               _CLK_HSICmd:
 168                     ; 157   assert_param(IS_FUNCTIONAL_STATE(NewState));
 170                     ; 159   if (NewState != DISABLE)
 172  0041 4d            	tnz	a
 173  0042 2706          	jreq	L74
 174                     ; 162     CLK->ICKCR |= CLK_ICKCR_HSION;
 176  0044 721050c2      	bset	20674,#0
 178  0048 2004          	jra	L15
 179  004a               L74:
 180                     ; 167     CLK->ICKCR &= (uint8_t)(~CLK_ICKCR_HSION);
 182  004a 721150c2      	bres	20674,#0
 183  004e               L15:
 184                     ; 169 }
 187  004e 81            	ret
 223                     ; 182 void CLK_AdjustHSICalibrationValue(uint8_t CLK_HSICalibrationValue)
 223                     ; 183 {
 224                     	switch	.text
 225  004f               _CLK_AdjustHSICalibrationValue:
 229                     ; 185   CLK->HSIUNLCKR = 0xAC;
 231  004f 35ac50ce      	mov	20686,#172
 232                     ; 186   CLK->HSIUNLCKR = 0x35;
 234  0053 353550ce      	mov	20686,#53
 235                     ; 189   CLK->HSITRIMR = (uint8_t)CLK_HSICalibrationValue;
 237  0057 c750cd        	ld	20685,a
 238                     ; 190 }
 241  005a 81            	ret
 276                     ; 206 void CLK_LSICmd(FunctionalState NewState)
 276                     ; 207 {
 277                     	switch	.text
 278  005b               _CLK_LSICmd:
 282                     ; 210   assert_param(IS_FUNCTIONAL_STATE(NewState));
 284                     ; 212   if (NewState != DISABLE)
 286  005b 4d            	tnz	a
 287  005c 2706          	jreq	L701
 288                     ; 215     CLK->ICKCR |= CLK_ICKCR_LSION;
 290  005e 721450c2      	bset	20674,#2
 292  0062 2004          	jra	L111
 293  0064               L701:
 294                     ; 220     CLK->ICKCR &= (uint8_t)(~CLK_ICKCR_LSION);
 296  0064 721550c2      	bres	20674,#2
 297  0068               L111:
 298                     ; 222 }
 301  0068 81            	ret
 363                     ; 243 void CLK_HSEConfig(CLK_HSE_TypeDef CLK_HSE)
 363                     ; 244 {
 364                     	switch	.text
 365  0069               _CLK_HSEConfig:
 369                     ; 246   assert_param(IS_CLK_HSE(CLK_HSE));
 371                     ; 250   CLK->ECKCR &= (uint8_t)~CLK_ECKCR_HSEON;
 373  0069 721150c6      	bres	20678,#0
 374                     ; 253   CLK->ECKCR &= (uint8_t)~CLK_ECKCR_HSEBYP;
 376  006d 721950c6      	bres	20678,#4
 377                     ; 256   CLK->ECKCR |= (uint8_t)CLK_HSE;
 379  0071 ca50c6        	or	a,20678
 380  0074 c750c6        	ld	20678,a
 381                     ; 257 }
 384  0077 81            	ret
 446                     ; 274 void CLK_LSEConfig(CLK_LSE_TypeDef CLK_LSE)
 446                     ; 275 {
 447                     	switch	.text
 448  0078               _CLK_LSEConfig:
 452                     ; 277   assert_param(IS_CLK_LSE(CLK_LSE));
 454                     ; 281   CLK->ECKCR &= (uint8_t)~CLK_ECKCR_LSEON;
 456  0078 721550c6      	bres	20678,#2
 457                     ; 284   CLK->ECKCR &= (uint8_t)~CLK_ECKCR_LSEBYP;
 459  007c 721b50c6      	bres	20678,#5
 460                     ; 287   CLK->ECKCR |= (uint8_t)CLK_LSE;
 462  0080 ca50c6        	or	a,20678
 463  0083 c750c6        	ld	20678,a
 464                     ; 289 }
 467  0086 81            	ret
 491                     ; 300 void CLK_ClockSecuritySystemEnable(void)
 491                     ; 301 {
 492                     	switch	.text
 493  0087               _CLK_ClockSecuritySystemEnable:
 497                     ; 303   CLK->CSSR |= CLK_CSSR_CSSEN;
 499  0087 721050ca      	bset	20682,#0
 500                     ; 304 }
 503  008b 81            	ret
 539                     ; 311 void CLK_ClockSecuritySytemDeglitchCmd(FunctionalState NewState)
 539                     ; 312 {
 540                     	switch	.text
 541  008c               _CLK_ClockSecuritySytemDeglitchCmd:
 545                     ; 314   assert_param(IS_FUNCTIONAL_STATE(NewState));
 547                     ; 316   if (NewState != DISABLE)
 549  008c 4d            	tnz	a
 550  008d 2706          	jreq	L122
 551                     ; 319     CLK->CSSR |= CLK_CSSR_CSSDGON;
 553  008f 721850ca      	bset	20682,#4
 555  0093 2004          	jra	L322
 556  0095               L122:
 557                     ; 324     CLK->CSSR &= (uint8_t)(~CLK_CSSR_CSSDGON);
 559  0095 721950ca      	bres	20682,#4
 560  0099               L322:
 561                     ; 326 }
 564  0099 81            	ret
 705                     ; 350 void CLK_CCOConfig(CLK_CCOSource_TypeDef CLK_CCOSource, CLK_CCODiv_TypeDef CLK_CCODiv)
 705                     ; 351 {
 706                     	switch	.text
 707  009a               _CLK_CCOConfig:
 709  009a 89            	pushw	x
 710       00000000      OFST:	set	0
 713                     ; 353   assert_param(IS_CLK_OUTPUT(CLK_CCOSource));
 715                     ; 354   assert_param(IS_CLK_OUTPUT_DIVIDER(CLK_CCODiv));
 717                     ; 357   CLK->CCOR = (uint8_t)((uint8_t)CLK_CCOSource | (uint8_t)CLK_CCODiv);
 719  009b 9f            	ld	a,xl
 ...
 725  00a1 85            	popw	x
 726  00a2 81            	ret
 800                     ; 410 void CLK_SYSCLKSourceConfig(CLK_SYSCLKSource_TypeDef CLK_SYSCLKSource)
 800                     ; 411 {
 801                     	switch	.text
 802  00a3               _CLK_SYSCLKSourceConfig:
 806                     ; 413   assert_param(IS_CLK_SOURCE(CLK_SYSCLKSource));
 808                     ; 416   CLK->SWR = (uint8_t)CLK_SYSCLKSource;
 810  00a3 c750c8        	ld	20680,a
 811                     ; 417 }
 814  00a6 81            	ret
 839                     ; 429 CLK_SYSCLKSource_TypeDef CLK_GetSYSCLKSource(void)
 839                     ; 430 {
 840                     	switch	.text
 841  00a7               _CLK_GetSYSCLKSource:
 845                     ; 431   return ((CLK_SYSCLKSource_TypeDef)(CLK->SCSR));
 847  00a7 c650c7        	ld	a,20679
 850  00aa 81            	ret
 922                     ; 472 uint32_t CLK_GetClockFreq(void)
 922                     ; 473 {
 923                     	switch	.text
 924  00ab               _CLK_GetClockFreq:
 926  00ab 5209          	subw	sp,#9
 927       00000009      OFST:	set	9
 930                     ; 474   uint32_t clockfrequency = 0;
 932                     ; 475   uint32_t sourcefrequency = 0;
 ...
 950                     ; 482   if ( clocksource == CLK_SYSCLKSource_HSI)
 952  00bc 7b09          	ld	a,(OFST+0,sp)
 953  00be a101          	cp	a,#1
 954  00c0 260c          	jrne	L704
 955                     ; 484     sourcefrequency = HSI_VALUE;
 ...
 964  00ce               L704:
 965                     ; 486   else if ( clocksource == CLK_SYSCLKSource_LSI)
 967  00ce 7b09          	ld	a,(OFST+0,sp)
 968  00d0 a102          	cp	a,#2
 969  00d2 260c          	jrne	L314
 970                     ; 488     sourcefrequency = LSI_VALUE;
...
1036                     ; 506   return((uint32_t)clockfrequency);
1038  0126 96            	ldw	x,sp
1039  0127 1c0005        	addw	x,#OFST-4
1040  012a cd0000        	call	c_ltor
1044  012d 5b09          	addw	sp,#9
1045  012f 81            	ret
1143                     ; 522 void CLK_SYSCLKDivConfig(CLK_SYSCLKDiv_TypeDef CLK_SYSCLKDiv)
1143                     ; 523 {
1144                     	switch	.text
1145  0130               _CLK_SYSCLKDivConfig:
1149                     ; 525   assert_param(IS_CLK_SYSTEM_DIVIDER(CLK_SYSCLKDiv));
1151                     ; 527   CLK->CKDIVR = (uint8_t)(CLK_SYSCLKDiv);
1153  0130 c750c0        	ld	20672,a
1154                     ; 528 }
1157  0133 81            	ret
1193                     ; 535 void CLK_SYSCLKSourceSwitchCmd(FunctionalState NewState)
1193                     ; 536 {
1194                     	switch	.text
1195  0134               _CLK_SYSCLKSourceSwitchCmd:
1199                     ; 538   assert_param(IS_FUNCTIONAL_STATE(NewState));
1201                     ; 540   if (NewState != DISABLE)
1203  0134 4d            	tnz	a
1204  0135 2706          	jreq	L305
1205                     ; 543     CLK->SWCR |= CLK_SWCR_SWEN;
1207  0137 721250c9      	bset	20681,#1
1209  013b 2004          	jra	L505
1210  013d               L305:
1211                     ; 548     CLK->SWCR &= (uint8_t)(~CLK_SWCR_SWEN);
1213  013d 721350c9      	bres	20681,#1
1214  0141               L505:
1215                     ; 550 }
1218  0141 81            	ret
1364                     ; 610 void CLK_RTCClockConfig(CLK_RTCCLKSource_TypeDef CLK_RTCCLKSource, CLK_RTCCLKDiv_TypeDef CLK_RTCCLKDiv)
1364                     ; 611 {
1365                     	switch	.text
1366  0142               _CLK_RTCClockConfig:
1368  0142 89            	pushw	x
1369       00000000      OFST:	set	0
1372                     ; 613   assert_param(IS_CLK_CLOCK_RTC(CLK_RTCCLKSource));
1374                     ; 614   assert_param(IS_CLK_CLOCK_RTC_DIV(CLK_RTCCLKDiv));
1376                     ; 617   CLK->CRTCR = (uint8_t)((uint8_t)CLK_RTCCLKSource | (uint8_t)CLK_RTCCLKDiv);
...
1384  0149 85            	popw	x
1385  014a 81            	ret
1451                     ; 629 void CLK_BEEPClockConfig(CLK_BEEPCLKSource_TypeDef CLK_BEEPCLKSource)
1451                     ; 630 {
1452                     	switch	.text
1453  014b               _CLK_BEEPClockConfig:
1457                     ; 632   assert_param(IS_CLK_CLOCK_BEEP(CLK_BEEPCLKSource));
1459                     ; 635   CLK->CBEEPR = (uint8_t)(CLK_BEEPCLKSource);
1461  014b c750cb        	ld	20683,a
1462                     ; 637 }
1465  014e 81            	ret
1690                     ; 671 void CLK_PeripheralClockConfig(CLK_Peripheral_TypeDef CLK_Peripheral, FunctionalState NewState)
1690                     ; 672 {
1691                     	switch	.text
...
1699                     ; 673   uint8_t reg = 0;
1701                     ; 676   assert_param(IS_CLK_PERIPHERAL(CLK_Peripheral));
1703                     ; 677   assert_param(IS_FUNCTIONAL_STATE(NewState));
1705                     ; 680   reg = (uint8_t)((uint8_t)CLK_Peripheral & (uint8_t)0xF0);
1707  0151 9e            	ld	a,xh
1708  0152 a4f0          	and	a,#240
1709  0154 6b01          	ld	(OFST+0,sp),a
1711                     ; 682   if ( reg == 0x00)
1713  0156 0d01          	tnz	(OFST+0,sp)
1714  0158 2635          	jrne	L527
1715                     ; 684     if (NewState != DISABLE)
1717  015a 0d03          	tnz	(OFST+2,sp)
1718  015c 2719          	jreq	L727
1719                     ; 687       CLK->PCKENR1 |= (uint8_t)((uint8_t)1 << ((uint8_t)CLK_Peripheral & (uint8_t)0x0F));
...
1737  0177               L727:
1738                     ; 692       CLK->PCKENR1 &= (uint8_t)(~(uint8_t)(((uint8_t)1 << ((uint8_t)CLK_Peripheral & (uint8_t)0x0F))));
...
1756  018f               L527:
1757                     ; 695   else if (reg == 0x10)
1759  018f 7b01          	ld	a,(OFST+0,sp)
1760  0191 a110          	cp	a,#16
1761  0193 2633          	jrne	L537
1762                     ; 697     if (NewState != DISABLE)
1764  0195 0d03          	tnz	(OFST+2,sp)
1765  0197 2717          	jreq	L737
1766                     ; 700       CLK->PCKENR2 |= (uint8_t)((uint8_t)1 << ((uint8_t)CLK_Peripheral & (uint8_t)0x0F));
...
1784  01b0               L737:
1785                     ; 705       CLK->PCKENR2 &= (uint8_t)(~(uint8_t)(((uint8_t)1 << ((uint8_t)CLK_Peripheral & (uint8_t)0x0F))));
...
1803  01c8               L537:
1804                     ; 710     if (NewState != DISABLE)
1806  01c8 0d03          	tnz	(OFST+2,sp)
1807  01ca 2717          	jreq	L547
1808                     ; 713       CLK->PCKENR3 |= (uint8_t)((uint8_t)1 << ((uint8_t)CLK_Peripheral & (uint8_t)0x0F));
...
1826  01e3               L547:
1827                     ; 718       CLK->PCKENR3 &= (uint8_t)(~(uint8_t)(((uint8_t)1 << ((uint8_t)CLK_Peripheral & (uint8_t)0x0F))));
...
1849  01fb 81            	ret
1873                     ; 759 void CLK_LSEClockSecuritySystemEnable(void)
1873                     ; 760 {
1874                     	switch	.text
1875  01fc               _CLK_LSEClockSecuritySystemEnable:
1879                     ; 762   CSSLSE->CSR |= CSSLSE_CSR_CSSEN;
1881  01fc 72105190      	bset	20880,#0
1882                     ; 763 }
1885  0200 81            	ret
1909                     ; 771 void CLK_RTCCLKSwitchOnLSEFailureEnable(void)
1909                     ; 772 {
1910                     	switch	.text
1911  0201               _CLK_RTCCLKSwitchOnLSEFailureEnable:
1915                     ; 774   CSSLSE->CSR |= CSSLSE_CSR_SWITCHEN;
1917  0201 72125190      	bset	20880,#1
1918                     ; 775 }
1921  0205 81            	ret
1996                     ; 801 void CLK_HaltConfig(CLK_Halt_TypeDef CLK_Halt, FunctionalState NewState)
1996                     ; 802 {
1997                     	switch	.text
1998  0206               _CLK_HaltConfig:
2000  0206 89            	pushw	x
2001       00000000      OFST:	set	0
2004                     ; 804   assert_param(IS_CLK_HALT(CLK_Halt));
2006                     ; 805   assert_param(IS_FUNCTIONAL_STATE(NewState));
2008                     ; 807   if (NewState != DISABLE)
2010  0207 9f            	ld	a,xl
2011  0208 4d            	tnz	a
2012  0209 2709          	jreq	L5201
2013                     ; 809     CLK->ICKCR |= (uint8_t)(CLK_Halt);
2015  020b 9e            	ld	a,xh
2016  020c ca50c2        	or	a,20674
2017  020f c750c2        	ld	20674,a
2019  0212 2009          	jra	L7201
2020  0214               L5201:
2021                     ; 813     CLK->ICKCR &= (uint8_t)(~CLK_Halt);
...
2471                     ; 941   uint8_t reg = 0;
2473                     ; 942   uint8_t pos = 0;
2475                     ; 943   FlagStatus bitstatus = RESET;
2477                     ; 946   assert_param(IS_CLK_FLAGS(CLK_FLAG));
2479                     ; 949   reg = (uint8_t)((uint8_t)CLK_FLAG & (uint8_t)0xF0);
2481  026f a4f0          	and	a,#240
2482  0271 6b02          	ld	(OFST+0,sp),a
2484                     ; 952   pos = (uint8_t)((uint8_t)CLK_FLAG & (uint8_t)0x0F);
2486  0273 7b03          	ld	a,(OFST+1,sp)
2487  0275 a40f          	and	a,#15
2488  0277 6b01          	ld	(OFST-1,sp),a
2490                     ; 954   if (reg == 0x00) /* The flag to check is in CRTC Rregister */
2492  0279 0d02          	tnz	(OFST+0,sp)
2493  027b 2607          	jrne	L7421
2494                     ; 956     reg = CLK->CRTCR;
...
2614                     ; 994     bitstatus = SET;
2616  02f5 a601          	ld	a,#1
2617  02f7 6b02          	ld	(OFST+0,sp),a
2620  02f9 2002          	jra	L1131
2621  02fb               L7031:
2622                     ; 998     bitstatus = RESET;
2624  02fb 0f02          	clr	(OFST+0,sp)
2626  02fd               L1131:
2627                     ; 1002   return((FlagStatus)bitstatus);
2629  02fd 7b02          	ld	a,(OFST+0,sp)
2632  02ff 5b03          	addw	sp,#3
2633  0301 81            	ret
2656                     ; 1010 void CLK_ClearFlag(void)
2656                     ; 1011 {
2657                     	switch	.text
2658  0302               _CLK_ClearFlag:
2662                     ; 1014   CSSLSE->CSR &= (uint8_t)(~CSSLSE_CSR_CSSF);
2664  0302 72175190      	bres	20880,#3
2665                     ; 1015 }
2668  0306 81            	ret
2714                     ; 1026 ITStatus CLK_GetITStatus(CLK_IT_TypeDef CLK_IT)
2714                     ; 1027 {
2715                     	switch	.text
